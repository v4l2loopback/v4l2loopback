...
#define dprintk(fmt, args...)                                                  \
  do {                                                                         \
    if (debug > 0) {                                                           \
      printk(KERN_INFO                                                         \
             "v4l2-loopback[" __stringify(__LINE__) "], pid(%d):  " fmt,       \
...
#else
static inline int v4l2l_mod64(const s64 A, const u32 B) { return A % B; }
#endif
...
MODULE_PARM_DESC(
    ...
    "[DEFAULT: " __stringify(V4L2LOOPBACK_DEFAULT_EXCLUSIVECAPS) "]");
...
static int v4l2l_fill_format(struct v4l2_format *fmt, const u32 minwidth,
  ...
  if (0) {
  ...
  } else if (!v4l2_fill_pixfmt(&fmt0.fmt.pix, pixelformat, width, height)) {
    ;
    ...
    const struct v4l2l_format *format = format_by_fourcc(pixelformat);
    if (!format)
      return -EINVAL;
...
static void set_timeperframe(struct v4l2_loopback_device *dev,
  ...
  dev->frame_jiffies =
      max(1UL, (msecs_to_jiffies(1000) * tpf->numerator) / tpf->denominator);
}
...
static int v4l2loopback_lookup(int device_nr,
  ...
  int err =
      idr_for_each(&v4l2loopback_index_idr, &v4l2loopback_lookup_cb, &data);
  if (1 == err) {
    if (device)
      *device = data.device;
...
static int vidioc_querycap(struct file *file, void *fh,
  ...
  snprintf(cap->bus_info, sizeof(cap->bus_info), "platform:v4l2loopback-%03d",
           device_nr);
...
static int vidioc_enum_frameintervals(struct file *file, void *fh,
  ...
  if (dev->keep_format || has_other_owners(opener, dev)) {
    ...
    if (argp->width < dev->min_width || argp->width > dev->max_width ||
        argp->height < dev->min_height || argp->height > dev->max_height ||
...
static int vidioc_try_fmt_vid(struct file *file, void *fh,
  ...
  if (check_buffer_capability(dev, opener, f->type) < 0)
    return -EINVAL;
  if (v4l2l_fill_format(f, dev->min_width, dev->max_width, dev->min_height,
                        dev->max_height) != 0)
    return -EINVAL;
  if (dev->keep_format || has_other_owners(opener, dev))
 ...
 */
static int vidioc_s_fmt_vid(struct file *file, void *fh,
                            struct v4l2_format *f) {
  struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
  struct v4l2_loopback_opener *opener = fh_to_opener(fh);
  u32 token = opener->io_method == V4L2L_IO_TIMEOUT ? V4L2L_TOKEN_TIMEOUT
...
static int v4l2loopback_s_ctrl(struct v4l2_ctrl *ctrl) {
  struct v4l2_loopback_device *dev =
      container_of(ctrl->handler, struct v4l2_loopback_device, ctrl_handler);
...
#define is_allocated(opener, type, index)                                      \
  (opener->format_token &                                                      \
       (opener->io_method == V4L2L_IO_TIMEOUT ? V4L2L_TOKEN_TIMEOUT            \
...
#define BUFFER_DEBUG_FMT_ARGS(buf)                                             \
  (buf)->index, (buf), (buf)->type, (buf)->bytesused, (buf)->length,           \
      (buf)->flags, (buf)->field, (long long)(buf)->timestamp.tv_sec,          \
...
static int vidioc_reqbufs(struct file *file, void *fh,
  ...
  dprintk("REQBUFS(memory=%u, req_count=%u) and device-bufs=%u/%u "
          "[used/max]\n",
          reqbuf->memory, req_count, dev->used_buffer_count, dev->buffer_count);
  ...
  case V4L2_BUF_TYPE_VIDEO_CAPTURE:
  case V4L2_BUF_TYPE_VIDEO_OUTPUT:
    if (!(dev->format_tokens & token || opener->format_token & token))
...
static void buffer_written(struct v4l2_loopback_device *dev,
  ...
  spin_lock_bh(&dev->lock);
  dev->bufpos2index[v4l2l_mod64(dev->write_position, dev->used_buffer_count)] =
      buf->buffer.index;
  ++dev->write_position;
  dev->reread_count = 0;
...
static int vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf) {
  ...
  switch (type) {
  case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    dprintkrw("QBUF(CAPTURE, index=%u) -> " BUFFER_DEBUG_FMT_STR, index,
              BUFFER_DEBUG_FMT_ARGS(buf));
    set_queued(buf->flags);
    break;
  case V4L2_BUF_TYPE_VIDEO_OUTPUT:
    dprintkrw("QBUF(OUTPUT, index=%u) -> " BUFFER_DEBUG_FMT_STR, index,
              BUFFER_DEBUG_FMT_ARGS(buf));
    ...
    if (dev->pix_format_has_valid_sizeimage) {
      if (buf->bytesused >= dev->pix_format.sizeimage) {
        bufd->buffer.bytesused = dev->pix_format.sizeimage;
      } else {
...
static int get_capture_buffer(struct file *file) {
  ...
  if (dev->write_position == opener->read_position) {
    ...
    ++opener->reread_count;
    pos = v4l2l_mod64(opener->read_position + dev->used_buffer_count - 1,
                      dev->used_buffer_count);
  } else {
    opener->reread_count = 0;
    if (dev->write_position > opener->read_position + dev->used_buffer_count)
      opener->read_position = dev->write_position - 1;
    pos = v4l2l_mod64(opener->read_position, dev->used_buffer_count);
  ...
  if (timeout_happened) {
    ...
    memcpy(dev->image + dev->buffers[index].buffer.m.offset, dev->timeout_image,
           dev->buffer_size);
  }
  return (int)index;
}
...
static int vidioc_dqbuf(struct file *file, void *fh, struct v4l2_buffer *buf) {
  ...
  case V4L2_BUF_TYPE_VIDEO_OUTPUT:
    ...
    bufd = list_first_entry_or_null(&dev->outbufs_list, struct v4l2l_buffer,
                                    list_head);
    if (bufd)
...
static void client_usage_queue_event(struct video_device *vdev) {
  ...
  dev = container_of(vdev->v4l2_dev, struct v4l2_loopback_device, v4l2_dev);
...
static unsigned int v4l2_loopback_poll(struct file *file,
  ...
  case V4L2L_TOKEN_OUTPUT:
    if (opener->stream_token != 0 || opener->io_method == V4L2L_IO_NONE)
      ret_mask |= POLLOUT | POLLWRNORM;
    break;
  case V4L2L_TOKEN_CAPTURE:
    if ((opener->io_method == V4L2L_IO_NONE || opener->stream_token != 0) &&
        can_read(dev, opener))
...
static int v4l2_loopback_close(struct file *file) {
  ...
  if (opener->format_token) {
    ...
    if (reqbuf.type)
      result = vidioc_reqbufs(file, file->private_data, &reqbuf);
    if (result < 0)
...
static int start_fileio(struct file *file, void *fh, enum v4l2_buf_type type) {
  ...
  struct v4l2_loopback_opener *opener = fh_to_opener(fh);
  struct v4l2_requestbuffers reqbuf = {
      .count = dev->buffer_count, .memory = V4L2_MEMORY_MMAP, .type = type};
...
static ssize_t v4l2_loopback_read(struct file *file, char __user *buf,
  ...
  result = start_fileio(file, file->private_data, V4L2_BUF_TYPE_VIDEO_CAPTURE);
  if (result < 0)
    return result;
...
static ssize_t v4l2_loopback_write(struct file *file, const char __user *buf,
  ...
  result = start_fileio(file, file->private_data, V4L2_BUF_TYPE_VIDEO_OUTPUT);
  if (result < 0)
    return result;
...
static void free_buffers(struct v4l2_loopback_device *dev) {
  ...
  if (!has_no_owners(dev) || any_buffers_mapped(dev))
    ...
    printk(KERN_WARNING "v4l2-loopback free_buffers() buffers of video device "
...
static void free_timeout_buffer(struct v4l2_loopback_device *dev) {
  ...
  if ((dev->timeout_jiffies > 0 && !has_no_owners(dev)) ||
    ...
    printk(KERN_WARNING "v4l2-loopback free_timeout_buffer() timeout image "
...
#define DEFAULT_FROM_CONF(confmember, default_condition, default_value)        \
  ((conf) ? ((conf->confmember default_condition) ? (default_value)            \
                                                  : (conf->confmember))        \
          : default_value)
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr) {
  ...
  u32 _min_width = DEFAULT_FROM_CONF(min_width, < V4L2LOOPBACK_SIZE_MIN_WIDTH,
                                     V4L2LOOPBACK_SIZE_MIN_WIDTH);
  u32 _min_height = DEFAULT_FROM_CONF(
      min_height, < V4L2LOOPBACK_SIZE_MIN_HEIGHT, V4L2LOOPBACK_SIZE_MIN_HEIGHT);
  u32 _max_width = DEFAULT_FROM_CONF(max_width, < _min_width, max_width);
  u32 _max_height = DEFAULT_FROM_CONF(max_height, < _min_height, max_height);
  ...
  if (conf) {
    ...
    if (capture_nr >= 0 && output_nr == capture_nr) {
      ...
      printk(KERN_ERR "v4l2-loopback add() split OUTPUT and CAPTURE "
                      "devices not yet supported.\n");
      printk(KERN_INFO "v4l2-loopback add() both devices must have the "
                       "same number (%d != %d).\n",
  ...
  snprintf(dev->v4l2_dev.name, sizeof(dev->v4l2_dev.name), "v4l2loopback-%03d",
           nr);
  ...
  _fmt = (struct v4l2_format){.type = V4L2_BUF_TYPE_VIDEO_CAPTURE,
  ...
  if (video_register_device(dev->vdev, VFL_TYPE_VIDEO, nr) < 0) {
    printk(KERN_ERR "v4l2-loopback add() failed video_register_device()\n");
    err = -EFAULT;
...
static long v4l2loopback_control_ioctl(struct file *file, unsigned int cmd,
  ...
  case V4L2LOOPBACK_CTL_REMOVE_legacy:
     ...
     * this is mostly about limits (which cannot be queried directly with
     * VIDIOC_G_FMT and friends
     */
  ...
  case V4L2LOOPBACK_CTL_QUERY_legacy:
    ...
    if ((device_nr != capture_nr) && (capture_nr >= 0) &&
      ...
      break;
    MARK();
    /* if otoh, we got the device from capture_nr and there is a valid
     * output_nr, make sure that both refer to the same device (or bail out)
     */
...
static int __init v4l2loopback_init_module(void) {
  ...
  if (devices > MAX_DEVICES) {
    devices = MAX_DEVICES;
    printk(KERN_INFO "v4l2-loopback init() number of initial devices is "
                     "limited to: %d\n",
           MAX_DEVICES);
  }
  ...
  if (max_buffers > MAX_BUFFERS) {
    ...
    printk(KERN_INFO "v4l2-loopback init() number of buffers is limited "
                     "to: %d\n",
           MAX_BUFFERS);
  }

  if (max_openers < 0) {
    printk(KERN_INFO "v4l2-loopback init() allowing %d openers rather "
                     "than %d\n",
           2, max_openers);
    max_openers = 2;
  }
