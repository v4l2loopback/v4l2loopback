...
MODULE_PARM_DESC(dynamic_buffering, "Control Dynamic Buffering (0=Disable, 1=Enable, [Default=0]");
    ...
    do { \
        ...
        if (releasing_token) { \
            ...
            dprintk("release_token: opener=%p released token=0x%x (dynamic=%d)\n", \
                    opener, releasing_token, (dev)->use_dynamic_buffering); \
        } \
        spin_unlock_irqrestore(&(dev)->lock, flags); \
    } while (0)
...
static int resize_dynamic_buffer(struct v4l2_loopback_device *dev, u32 new_capacity);
static int dynamic_buffer_write(struct v4l2_loopback_device *dev, const u8 *src, u32 len);
static int dynamic_buffer_read(struct v4l2_loopback_device *dev, u8 *dst, u32 len, bool block);
static int sync_dynamic_to_v4l2_buffer(struct v4l2_loopback_device *dev,struct v4l2l_buffer *bufd, u32 bytesused);
static int __resize_buffer_locked(struct dynamic_buffer *dbuf, u32 new_capacity);
static void __copy_circular_data(const void *src_data, u32 src_size, u32 src_pos,void *dst_data, u32 copy_len);
 ...
 */
static DEVICE_ATTR(dynamic_buffering, S_IRUGO | S_IWUSR,attr_show_dynamic_buffering, attr_store_dynamic_buffering);
                                        ...
                                        char *buf)
{
    ...
    len += sprintf(buf + len, "=== V4L2-Loopback Dynamic Buffer Statistics ===\n\n");
	...
	format_buffer_size(dbuf->stats.total_bytes_written, size_str, sizeof(size_str));
	len += sprintf(buf + len, "Total written:   %s\n", size_str);
	format_buffer_size(dbuf->stats.total_bytes_read, size_str, sizeof(size_str));
	...
	format_buffer_size(dbuf->stats.max_capacity_reached, size_str, sizeof(size_str));
	len += sprintf(buf + len, "Max capacity:    %s\n", size_str);
	
	/* Timestamps of last operations */
	if (dbuf->stats.last_expand_at) {
		s64 ago = ktime_ms_delta(now, dbuf->stats.last_expand_at) / 1000;
		len += sprintf(buf + len, "Last expand:     %lld seconds ago\n", ago);
	}
	if (dbuf->stats.last_shrink_at) {
		s64 ago = ktime_ms_delta(now, dbuf->stats.last_shrink_at) / 1000;
...
#define V4L2_SYSFS_DESTROY(x) device_remove_file(&vdev->dev, &dev_attr_##x)
	...
	if (vdev) {
		...
		V4L2_SYSFS_DESTROY(dynamic_buffering); // Dynamic buffer implementation
		V4L2_SYSFS_DESTROY(dynamic_buffer_stats); // Dynamic buffer implementation
		V4L2_SYSFS_DESTROY(dynamic_buffer_reset_stats); //Dynamic buffer implementation
		/* ... */
	}
}
...
#define V4L2_SYSFS_CREATE(x)                                 \
	...
	do {
		...
		V4L2_SYSFS_CREATE(dynamic_buffering);  // Dynamic buffer implementation
		V4L2_SYSFS_CREATE(dynamic_buffer_stats); // Dynamic buffer implementation
		V4L2_SYSFS_CREATE(dynamic_buffer_reset_stats); // Dynamic buffer implementation
...
static void format_buffer_size(size_t size, char *buf, size_t buf_size)
{
    ...
    if (unit_index == 0) {
        ...
        size_t decimal_part = (remainder * 100) / 1024; /* Two decimal places */
        if (decimal_part == 0) {
            snprintf(buf, buf_size, "%zu %s", size_copy, units[unit_index]);
        } else {
            snprintf(buf, buf_size, "%zu.%02zu %s", 
                    size_copy, decimal_part, units[unit_index]);
        }
    }
}
...
static int vidioc_s_fmt_vid(struct file *file, void *fh, struct v4l2_format *f)
{
    ...
    u32 token = opener->io_method == V4L2L_IO_TIMEOUT ?
                V4L2L_TOKEN_TIMEOUT : token_from_type(f->type);
    int changed, result;
    char buf[5];
    ...
    dprintk("S_FMT[%s] %4s:%ux%u size=%u (dynamic=%d)\n",
            ...
            f->fmt.pix.sizeimage, dynamic_mode);
    ...
    if (changed) {
        ...
        dev->pix_format_has_valid_sizeimage = v4l2l_pix_format_has_valid_sizeimage(f);
			     ...
			     struct v4l2_input *inp)
{
	...
	dprintk("vidioc_enum_input: stream_tokens=0x%x, keep_format=%d, dynamic=%d\n",
	        dev->stream_tokens, dev->keep_format, dev->use_dynamic_buffering);
                                      ...
                                      u32 bytesused)
{
    ...
    /* Reference release */
    if (atomic_dec_return(&dbuf->ref_count) == 0 && dbuf->shutdown_requested) {
        vfree(dbuf->data);
        kfree(dbuf);
    }
...
static int vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		...
		if (dev->use_dynamic_buffering && dev->dbuf) {
			...
			if (bytes_to_write > 0) {
				int ret = dynamic_buffer_write(dev, src, bytes_to_write);
				if (ret < 0) {
...
static int get_capture_buffer(struct file *file)
{
	...
	/* === ADDED THIS BLOCK === */
		...
		int sync_ret = sync_dynamic_to_v4l2_buffer(dev, 
		                                          &dev->buffers[index],
		                                          dev->buffer_size);
		if (sync_ret < 0) {
    ...
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
        if (dynamic_mode) {
            ...
            if (!(opener->stream_token & token)) {
                ...
                pr_info("v4l2-loopback: OUTPUT stream started (dynamic mode)\n");
            }
            result = 0;
        } else {
            /* Static mode: traditional validation */
...
static int init_dynamic_buffer(struct v4l2_loopback_device *dev)
{
    ...
    if (!dev->buffer_size) {
        if (dev->pix_format.sizeimage > 0) {
            ...
            dev->buffer_size = PAGE_ALIGN(640 * 480 * 2); /* YUYV 640x480 */
            dev->buffer_count = 2;
        }
    }

    /* Safe size calculation */
    if (unlikely(check_mul_overflow(dev->buffer_size, dev->buffer_count, &total_size))) {
 ...
 */
static void __copy_circular_data(const void *src_data, u32 src_size, u32 src_pos,
                                void *dst_data, u32 copy_len)
{
    ...
    if (first_chunk < copy_len) {
        memcpy((u8 *)dst_data + first_chunk, src_data, copy_len - first_chunk);
    }
}
...
static int __resize_buffer_locked(struct dynamic_buffer *dbuf, u32 new_capacity)
{
    ...
    pr_debug("v4l2-loopback: buffer resized from %u to %u bytes (data=%u)\n",
 ...
 */
static int dynamic_buffer_write(struct v4l2_loopback_device *dev,
...
retry_write:
    ...
    if (len <= free_space) {
        ...
        if (first_chunk < len) {
            memcpy(dbuf->data, src + first_chunk, len - first_chunk);
        }
    ...
    if (retry_count < MAX_RESIZE_RETRIES && 
        ...
        new_size = max(new_size, dbuf->size + len);  /* Make sure it fits */
    ...
    if (available > len) {
        u32 discard_amount = min(available / 2, available - len + (dbuf->size / 10));
    ...
    ret = wait_event_interruptible_timeout(dbuf->write_waitq,
 ...
 */
static int dynamic_buffer_read(struct v4l2_loopback_device *dev,
                              u8 *dst, u32 len, bool block)
{
    struct dynamic_buffer *dbuf;
    unsigned long flags;
    ...
    if (block) {
        ret = wait_event_interruptible(dbuf->read_waitq,
            (!dbuf->active || dbuf->shutdown_requested ||
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr)
{
	...
	if (conf && conf->dynamic_buffering == 1) {
		dev->use_dynamic_buffering = conf->dynamic_buffering; //Use Dynamic Buffer
	} else {
		dev->use_dynamic_buffering = dynamic_buffering; //Use Static Buffer (Default)
	}
...
static void v4l2_loopback_remove(struct v4l2_loopback_device *dev)
{
    ...
    if (dev->vdev) {
        struct v4l2loopback_private *priv = video_get_drvdata(dev->vdev);
        kfree(priv);
