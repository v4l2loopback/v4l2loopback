...
struct v4l2_loopback_config {
   ...
   * if output_nr and capture_nr are the same, only a single device will be
   * created NOTE: currently split-devices (where output_nr and capture_nr
   * differ) are not implemented yet. until then, requesting different
   * device-IDs will result in EINVAL.
   *
...
/* a pointer to a (struct v4l2_loopback_config) that has all values you wish to
 * impose on the to-be-created device set. if the ptr is NULL, a new device is
 * created with default values at the driver's discretion.
 *
 * returns the device_nr of the OUTPUT device (which can be used with
 * V4L2LOOPBACK_CTL_QUERY, to get more information on the device)
 */
 ...
 * capture_nr set (the two values must either refer to video-devices associated
 * with the same loopback device or exactly one of them must be <0
 */
